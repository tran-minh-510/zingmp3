    ...
    return (
        <div className="new-release">
            ...
            <div className="new-release_type">
                {btnType.map((item, index) => <div key={index} className={`btn-type ${indexList === index && 'active'}`} onClick={() => changeListType(index)}>{item}</div>)}
            </div>
            <div class="new-release_content pt-3">
                {listPlaylist?.map(
                    (item, index) => {
                        const { encodeId, title, artists, thumbnail, link, releaseDate } = item
                        return <Fragment key={index}>
                            <ItemsSong
                                data={{ encodeId, title, artists, link, thumbnail, releaseDate }}
                                icon={listIcon}
                            />
                        </Fragment>
                    }
